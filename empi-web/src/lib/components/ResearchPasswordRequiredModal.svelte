<script lang="ts">
	...
	if ($store.research_password) {
		const formData = new FormData();
		formData.set('current_password', $store.research_password);
...
<svelte:component this={Modal} bind:this={modal} show="{$store.research_password === ''}" dismissible={false}>
	<div slot="header">
				...
				use:enhance={() => {
					...
					return async ({result, formData}) => {
						...
						if (password_ok) {
							...
							if (password != null) {
								$store.research_password = password.toString();
								modal.dismiss();
							}
						}
					};
