...
class Email(models.Model):
    ...
    recipients = SeparatedValuesField(
        verbose_name="príjemcovia", field=models.EmailField
    )
    subject = models.CharField(max_length=78, verbose_name="predmet")
    body = models.TextField(verbose_name="telo")
    send_when = models.DateTimeField(
        verbose_name="dátum a čas odoslania", blank=True, null=True
    )
    is_finalized = models.BooleanField(
        verbose_name="je publikovaný", null=False, default=False
    )

    @classmethod
    def get_emails_to_send(cls):
        return cls.objects.filter(
            Q(send_when__isnull=True)
            | Q(send_when__lte=datetime.now()) & Q(is_finalized=True)
        )

    def send(self):
        html = render_to_string(
            "email.html", context={"subject": self.subject, "body": self.body}
        )
        attachments = [
            attachment.get_mimebase()
            for attachment in Attachment.objects.filter(email=self.pk)
        ]
...
class Attachment(models.Model):
    ...
    def get_mimebase(self):
        maintype, subtype = magic.from_file(self.file.path, mime=True).split(
            "/", maxsplit=1
        )
