...
class AttributeValueSerializer(serializers.ModelSerializer):
    attribute = serializers.HyperlinkedRelatedField(queryset=models.Attribute.objects.get_queryset(),
...
class ParticipantSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for pk, new_values in attributes.items():
            for value in new_values:
                try:
                    value = models.AttributeValue.objects.get(attribute_id=pk, value=value)
    ...
    def update(self, instance, validated_data):
        ...
        for chosen_value in chosen_values:
            ...
            attributes[pk].add(chosen_value['value'])
        for pk, new_values in attributes.items():
            current_values_queryset = instance.chosen_attribute_values.filter(attribute=pk)
            current_values: set[str] = {value.value for value in current_values_queryset}
